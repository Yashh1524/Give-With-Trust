...
const HomePageClient = ({ initialAmount, initialNgos, initialDonors }: Props) => {
    ...
    useEffect(() => {
        ...
        animateValue(initialAmount, setAmount);
        animateValue(initialNgos, setNgos);
        animateValue(initialDonors, setDonors);

        const interval = setInterval(() => {
            fetch("/api/total-raised")
                .then((res) => res.json())
                .then(({ totalAmountRaised, totalNGOCount, totalUserCount }) => {
                    animateValue(totalAmountRaised, setAmount);
                    animateValue(totalNGOCount, setNgos);
                    animateValue(totalUserCount, setDonors);
                });
        }, 5 * 60 * 1000); // every 5 minutes

        return () => clearInterval(interval);
    }, [initialAmount, initialNgos, initialDonors]);
